...
| Warehouse | `CRI_TRANSFORM_WH` — smallest practical size, auto-suspend on |
| Roles | `CRI_ADMIN`, `CRI_LOADER`, `CRI_TRANSFORMER`, `CRI_APP_READER` |

## Rules

- **Scripts are idempotent.** Re-running must not destroy or duplicate.
- **Least privilege.** `CRI_APP_READER` reads the curated surface only and must not be able to read `RAW`. Verify this explicitly after bootstrap.
